...
public class SellCommand implements CommandExecutor {
    ...
    public SellCommand(FoliaCore plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!player.hasPermission("foliacore.sell")) {
        ...
        if (item == null || item.getType() == Material.AIR) {
            plugin.getMessenger().sendError(player, "You must hold an item to sell.");
            return true;
        }
        ...
        if (price < 0) {
            ...
            if (item.getType() == Material.DIAMOND) price = 100.0;
                ...
                plugin.getMessenger().sendError(player, "This item cannot be sold.");
                return true;
            }
        }
        ...
        plugin.getMessenger().sendSuccess(player, "Sold " + amount + "x " + item.getType().name() + " for " + eco.format(total));
        return true;
    }
}
