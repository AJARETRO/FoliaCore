...
public class FoliaEconomy implements Economy {
    ...
    @Override
    public StringSingular() { return "Dollar"ayer player) {
    ...
    @Override publicolean hasAccount(OfflinePlayer player, String worldName) { return hasAccount(player); }
    ...
    @Override publi// File: src/main/java/dev/ajaretro/foliaCore/economy/FoliaEconomy.java
...
public class FoliaEconomy implements Economy {
    ...
    public EconomyResponse withdrawPlayer(OfflinePlayer player, double amount) {
        ...
        String uuid = player.getUniqueId().toString();
        EconomyResponse[] response = new EconomyResponse[1]; // Array hack to get value out of lambda

        balances.compute(uuid, (key, current) -> {
            if (current == null) current = 0.0;

            if (current < amount) {
                response[0] = new EconomyResponse(0, current, EconomyResponse.ResponseType.FAILURE, "Insufficient funds");
                return current; // Don't change balance
            }

            double newBalance = current - amount;
            response[0] = new EconomyResponse(amount, newBalance, EconomyResponse.ResponseType.SUCCESS, null);
            return newBalance; // Update map
        });
    ...
    @Override
    public EconomyResponse depositPlayer(OfflinePlayer player, double amount) {
        if (amount < 0) return new EconomyResponse(0, 0, EconomyResponse.ResponseType.FAILURE, "Canno
